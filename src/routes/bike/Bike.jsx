...
import {
	Box, Button, CitySelect, Content, DataTable, DateRange, dtUtils, Form, Input, Map, Modal, Select,
	utils
} from "beefly-common";
import env from "../../js/env";
import {
	bikeButteryStateMap, bikeMotorLockStateMap, bikeOnlineStateMap, bikeStateMap, loadButteryStateMap,
	orderFlowMap
} from '../../js/maps';
...
export default class Bike extends React.Component {
		...
		this.state = {
			columns: [
				...
				{title: '车辆位置', render: this.renderMap.bind(this)},
				{
					title: '电机状态',
					data: 'motorLockState',
					render: (data) => dtUtils.renderMap(data, bikeMotorLockStateMap)
				},
	...
	renderActions(data, type, row) {
		let actions = [
			{text: '鸣笛', onclick: `beefly.whistle('${row.userId}')`},
			{text: '车辆日志', onclick: `beefly.showMsg('${row.userId}')`},
			{text: '车辆详情', onclick: `beefly.details('${row.code}')`},
			...
			<Content>
				<Box theme="query">
					<Form inline>
						...
						<Select ref={(e) => this._isOnlineStatus = e} label="在线状态" options={bikeOnlineStateMap}
								whole={true}/>
						<CitySelect ref={(e) => this._citySelect = e} label='城市区域'/>
						<DateRange ref={(e) => this._dateRange = e} label="入库时间"/>
						<Select ref={(e) => this._loadButteryStatus = e} label="是否已装入电池" options={loadButteryStateMap}
								whole={true}/>
