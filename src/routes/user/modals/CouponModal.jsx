				...
				<Modal.Body>
					<Form className="form-label-100" horizontal>
						...
						<div className='user-block'>
							<Input label="出行券金额" type='number' model='query.couponAmout' validation={{required: true}}
								   width={250} placeholder='金额，正数'/>
							<Input label="出行券张数" type='number' model='query.num' validation={{required: true}}
								   width={80} placeholder='张数'/>
							{isOver &&
							<div className='text-red margin-l-10' style={{position: 'absolute', top: 110, left: 390}}>
								今日已到你可发送出行券的上限了，不可发送了！</div>}
						</div>
						<Input label="过期时间" type='date' model='query.expireTime' width={310}
